...
class BayesianDynamicFactorVec(PyMCStateSpace):
    r"""
    ...
    def make_symbolic_graph(self):
        ...
        # New order of states: all factor states first, then error states + all lags in same order (before factors then errors)
        self.ssm["design", :, :self.k_factors] = factor_loadings
        self.ssm["design", :, self.k_factors:self.k_factors + self.k_endog] = pt.eye(self.k_endog)
        ...
        if self.factor_order > 0 and self.error_order > 0:
            self.ssm["transition", self.k_factors + self.k_endog :(self.k_factors + self.k_endog) * (min_order), :(self.k_factors + self.k_endog)*(min_order-1)] = pt.eye((self.k_factors + self.k_endog) * (min_order -1))

        if self.factor_order > self.error_order:
            for i in range(self.factor_order - self.error_order):
                block_start = min_order * (self.k_factors + self.k_endog) + i * self.k_factors
                # Set AR coefficients for the top row of each factor AR(p) block
                self.ssm["transition", :self.k_factors, block_start:block_start + self.k_factors] = pt.diag(factor_ar[:, min_order + i])
            ...
            for i in range(self.error_order - self.factor_order):
                ...
                if i==0 and min_order == 1:
                    ...
                    self.ssm["transition", block_start:block_start + self.k_endog, min_order*(self.k_endog+self.k_factors):min_order*(self.k_endog+self.k_factors)+i*self.k_endog] = pt.eye(self.k_endog)
        ...
        #         self.k_factors + i,
        #     ] = 1.0
        self.ssm["selection", :self.k_factors+self.k_endog, :self.k_factors+self.k_endog] = pt.eye(self.k_factors+self.k_endog)
